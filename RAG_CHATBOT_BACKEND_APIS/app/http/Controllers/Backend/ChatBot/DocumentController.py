...
class   DocumentController:
    ...
    def upload_and_train(self, request, c_id):
        chat_id = c_id
        user_id = 1  # Hardcoded for now, but this could be dynamically fetched
        base_url = request.build_absolute_uri('/')
        url = f"{base_url}api/v1/upload/pdf/?chat_id={chat_id}&user_id={user_id}"
        payload = {}
        ...
        if request.method == 'POST' and len(request.FILES) != 0:
            ...
            for file_key in request.FILES:
                ...
                try:
                    ...
                    return JsonResponse({"status": "failed", "message": f"Error saving file {uploaded_file.name}"}, status=500)
            ...
            try:
                ...
                if response.status_code == 201:
                    ...
                    logger.error(f"Not Found error: {response_json}")
                    return JsonResponse({
                        "status": "failed",
